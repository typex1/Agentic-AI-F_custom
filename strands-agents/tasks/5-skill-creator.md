...
This builds directly on demo `06_agent-skill.py` (the `AgentSkills` plugin).
...
|--------|----------------|
| `strands-agents/06_agent-skill.py` | The `AgentSkills` plugin: `AgentSkills(skills=<dir>)`, `plugins=[...]`, `get_available_skills(...)`, `get_activated_skills(agent)`. Vendoring a skill's `SKILL.md` from GitHub. |
...
- **Discovery needs the agent.** Filesystem skills load *per-agent* at
  ...
  `asyncio.run(plugin.init_agent(agent))` before printing. (Demo 06 has this same
  latent quirk.)
