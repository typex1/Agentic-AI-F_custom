...
Builds on demos `05_mcp_tools.py` (consuming an MCP server) and `07_RAG_1.py`
(retrieval-augmented answering).

## Background

**RAG** = *retrieve* relevant context, then *generate* an answer grounded in it.
The retrieval step is the interesting part. Demo `07_RAG_1.py` does "structured
...
process. In demo 05 you *consumed* a remote MCP server. Here you'll *build* one:
...
- MCP server: `from mcp.server.fastmcp import FastMCP`, decorate a function with
  `@mcp.tool()`, and run `mcp.run()` (stdio). Client side (see demo 05 for the
...
- Compare this document-RAG approach with demo 07's NL2SQL "structured RAG":
