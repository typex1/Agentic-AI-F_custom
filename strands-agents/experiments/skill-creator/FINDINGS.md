...
| `try_skill_creator.py` | Strands runner: loads the skill via `AgentSkills` on Nova Lite (mirrors demo `06_agent-skill.py`) and drives it to author a new skill, then validates the result. |
...
- Demo `06_agent-skill.py` has the same latent gotcha — its discovery print
