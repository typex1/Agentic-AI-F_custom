...
Always follow this exact order.""",
    ...
    # Adjusted: pass only tool names (not full call records) — nova-lite fails to
    # serialize the scorer-tool input when given complex nested trajectories.
    trajectory = [
        t["name"]
        for t in tools_use_extractor.extract_agent_tools_used_from_messages(agent.messages)
    ]
...
evaluator = TrajectoryEvaluator(
    # Adjusted: LLM judge uses nova-lite (default judge model is not accessible)
    model=MODEL,
    rubric="""
...
sample_agent = Agent(model=MODEL, tools=[lookup_customer, get_order_history, process_refund])
