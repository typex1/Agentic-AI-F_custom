...
There are three patterns for this:
  1. Agents as Tools — one orchestrator calls specialists like functions (this example)
  2. Graph — explicit workflow with guaranteed execution order (next example)
...
orchestrator_model = BedrockModel(model_id="amazon.nova-lite-v1:0", region_name="us-east-1")
specialist_model = BedrockModel(model_id="amazon.nova-lite-v1:0", region_name="us-east-1")
...
result = writer("What are the latest Strands Agents SDK features? Check the official Github repo for Strands Agents")
print(result)
