...
from strands.vended_plugins.steering import (
    ...
    Proceed, Guide, Interrupt, ToolSteeringAction,
    LedgerProvider,
)
...
class SteeringDecision(BaseModel):
    ...
    Adjusted: replaces the framework-internal "_LLMSteering" model. Nova Lite
    drops the leading underscore when calling the structured-output tool
    ("_LLMSteering" -> "LLMSteering"), which makes the framework loop forever.
    An underscore-free name works reliably.
    """
...
If the agent violates any of these, provide specific guidance on what to fix."""
    ...
    async def steer_before_tool(self, *, agent, tool_use, **kwargs) -> ToolSteeringAction:
        """Adjusted: same logic as LLMSteeringHandler.steer_before_tool, but uses
        the underscore-free SteeringDecision model so Nova Lite's structured
        output tool call matches the registered tool name."""
        ...
        result = steering_agent(prompt, structured_output_model=SteeringDecision).structured_output

        if result.decision == "proceed":
            return Proceed(reason=result.reason)
        if result.decision == "guide":
            return Guide(reason=result.reason)
        return Interrupt(reason=result.reason)
